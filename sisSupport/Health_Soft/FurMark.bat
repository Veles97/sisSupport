...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: FurMark' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Проверка GPU, легендарный бублик' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\health_soft\FurMark"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка FurMark через winget...' -ForegroundColor Yellow"
winget install -e --id Geeks3D.FurMark.2 --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль FurMark успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b