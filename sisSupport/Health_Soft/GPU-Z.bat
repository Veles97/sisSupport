...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: GPU-Z' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Проверка GPU' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\health_soft\GPU-Z"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка GPU-Z через winget...' -ForegroundColor Yellow"
winget install -e --id TechPowerUp.GPU-Z --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль GPU-Z успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b