...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: OCCT' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Общая проверка ПК' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\health_soft\OCCT"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка OCCT через winget...' -ForegroundColor Yellow"
winget install -e --id OCBase.OCCT.Personal --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль OCCT успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b