...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: CPU-Z' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Проверка CPU' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\health_soft\CPU-Z"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка CPU-Z через winget...' -ForegroundColor Yellow"
winget install -e --id CPUID.CPU-Z --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль CPU-Z успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b