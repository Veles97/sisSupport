...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: IObitUnlocker' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Снимает зависшие фоновые процессы' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\health_soft\IObitUnlocker"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка IObitUnlocker через winget...' -ForegroundColor Yellow"
winget install -e --id IObit.IObitUnlocker --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль IObitUnlocker успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b