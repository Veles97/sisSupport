...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: WinDirStat' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Анализ использования дискового пространства' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\soft\WinDirStat"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка WinDirStat через winget...' -ForegroundColor Yellow"
:: Используем ключи для скрытия интерфейса и принятия соглашений
winget install -e --id WinDirStat.WinDirStat --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
    powershell -Command "Write-Host '[✗] Ошибка: Сбой установки через winget! (Возможно, winget не обновлен)' -ForegroundColor Red"
    pause > nul
    exit /b
)
...
powershell -Command "Write-Host '[✓] Модуль WinDirStat успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b