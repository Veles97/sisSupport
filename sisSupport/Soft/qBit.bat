...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: qBittorrent' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Торрент' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\soft\qBittorrent"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка qBittorrent через winget...' -ForegroundColor Yellow"
winget install -e --id qBittorrent.qBittorrent --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль qBittorrent успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b