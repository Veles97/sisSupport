...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: VLC Media Player' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Универсальный медиаплеер' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\soft\VLC"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка VLC через winget...' -ForegroundColor Yellow"
winget install -e --id VideoLAN.VLC --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль VLC Media Player успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b