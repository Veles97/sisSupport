...
powershell -Command "Write-Host '============================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '[➜] Программа: Rufus' -ForegroundColor Blue"
powershell -Command "Write-Host '[➜] Назначение: Создаёт загрузочную флешку' -ForegroundColor DarkGray"
echo.
...
set "DEFAULT_PATH=C:\Tools\soft\Rufus"
...
powershell -Command "Write-Host '[⚙] Скачивание и установка Rufus через winget...' -ForegroundColor Yellow"
winget install -e --id Rufus.Rufus --location "%USER_PATH%" --silent --accept-package-agreements --accept-source-agreements
if %errorlevel% neq 0 (
...
powershell -Command "Write-Host '[✓] Модуль Rufus успешно установлен!' -ForegroundColor Green"
explorer "%USER_PATH%"
exit /b